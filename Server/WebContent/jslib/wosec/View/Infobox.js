...
,   CSS_CLASS_INFOBOX_PIN_TEXT = "infobox-pin-text"
...
function getInfoboxPrototype(){
    if (!infoboxPrototype) {
        infoboxPrototype = $('<div class="' + CSS_CLASS_INFOBOX + '">' +
            "<span class='" + CSS_CLASS_INFOBOX_PIN_BUTTON + "'><span class='"+CSS_CLASS_INFOBOX_PIN_TEXT+"'>#</span></span>" +
        '</div>').hide();
    }
    ...
    return WoSec.extend(that, {
        ...
        setContent: function(information, task) {
            ...
            information.forEach(function(i) {
                ...
                entry.find("." + CSS_CLASS_INFOBOX_ENTRY_TIME).text(date.getHours() + ":" + date.getMinutes()+ ":" + date.getSeconds());
                if (i.participants) {
                ...
                if (i.attachments) {
                    ...
                    entry.find("."+CSS_CLASS_INFOBOX_ENTRY_ATTACHMENTS).remove();
                }
                ...
                if(!hasNoContent) {
                    ...
                    //alert(infobox.find("."+ CSS_CLASS_INFOBOX_ENTRY).length);
                    infobox.find("."+ CSS_CLASS_INFOBOX_PIN_BUTTON).after(entry);
                    /*var firstEntry = infobox.find("."+ CSS_CLASS_INFOBOX_ENTRY);
                    if (firstEntry.length == 0) {
                        infobox.append(entry);
                    } else {
                        firstEntry.parent().prepend(entry);
                    }*/
                    //
                }
            });
